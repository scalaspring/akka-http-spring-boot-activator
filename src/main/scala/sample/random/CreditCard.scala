...
object CreditCard {
  ...
  def random: CreditCard = random(brands.random)
  def random(brand: Brand): CreditCard = CreditCard(brand = brand, number = randomNumber(prefixes(brand).random, lengths(brand)), randomExpiration())
  ...
  protected def randomExpiration(days: Range = (3 * 30 to 36 * 30)): LocalDate = LocalDate.now().plusDays(days.random)
